...
// カスタム投稿タイプ: サブカテゴリー
function fascina_register_course_post_type() {
    $args = array(
        'public' => true,
        'label'  => 'サブカテゴリー',
        'labels' => array(
...
add_action('init', 'fascina_register_course_post_type');
...
// サブカテゴリー表示名を取得
function fascina_get_sub_category_display_name_by_value($main_category, $sub_category) {
    ...
    // フィルターを一時的に無効化
    global $wp_filter;
    $temp_filters = $wp_filter['pre_get_posts'] ?? null;
    if ($temp_filters) {
...
function fascina_get_nailist_choices_for_registration() {
    $choices = array();
...
// バナーの取得用関数
function fascina_get_banner_posts() {
    $args = array(
        'post_type' => 'banner',
        'posts_per_page' => -1,
        'post_status' => 'publish',
...
// カスタム投稿タイプ: 工程表
function fascina_register_process_chart_post_type() {
    $args = array(
...
add_action('init', 'fascina_register_process_chart_post_type');
        ...
        acf_add_local_field_group(array(
            ...
            'fields' => array(
                array(
                    ...
                    'choices' => fascina_get_nailist_choices_for_registration(),
                    'return_format' => 'value',
                    'layout' => 'vertical'
                ),
                array(
...
// ACFフィールドの選択肢を動的に更新
function fascina_load_gallery_sub_category_choices($field) {
    if ($field['name'] === 'gallery_sub_category') {
        ...
        if ($main_category) {
            $sub_categories = fascina_get_course_choices($main_category);
            $field['choices'] = $sub_categories;
        } else {
            // デフォルトでHandデザインのサブカテゴリーを表示
            $field['choices'] = fascina_get_course_choices('hand');
        }
    }
    return $field;
}
add_filter('acf/load_field/name=gallery_sub_category', 'fascina_load_gallery_sub_category_choices');
...
function fascina_remove_nailist_from_coupons_on_delete($post_id) {
...
add_action('before_delete_post', 'fascina_remove_nailist_from_coupons_on_delete');
add_action('trashed_post', 'fascina_remove_nailist_from_coupons_on_delete');

// サブカテゴリー削除時、紐づいているギャラリーのサブカテゴリーフィールドを空にして非表示にする
function fascina_clear_galleries_on_subcategory_delete($post_id) {
...
add_action('before_delete_post', 'fascina_clear_galleries_on_subcategory_delete');
add_action('trashed_post', 'fascina_clear_galleries_on_subcategory_delete');

// 新規投稿の作成日時を保存
function fascina_save_creation_date($post_id, $post, $update) {
...
add_action('wp_insert_post', 'fascina_save_creation_date', 10, 3);
...
// ブログアーカイブの表示件数を変更
function fascina_blog_query_settings($query)
{
...
add_action('pre_get_posts', 'fascina_blog_query_settings');

// 既存の投稿に作成日時を設定する関数
function fascina_set_creation_date_for_existing_posts() {
    $args = array(
...
add_action('after_switch_theme', 'fascina_set_creation_date_for_existing_posts');
...
add_action('admin_footer-post.php', 'fascina_gallery_category_script');
add_action('admin_footer-post-new.php', 'fascina_gallery_category_script');
...
function fascina_gallery_column_content($column_name, $post_id) {
    if ($column_name === 'thumbnail') {
        ...
        if ($main_category && $sub_category !== null) {
            echo esc_html(fascina_get_sub_category_display_name_by_value($main_category, $sub_category));
        }
...
function fascina_add_course_columns($columns) {
    unset($columns['title']);
    $new_columns = array();
    $new_columns['menu_order'] = '表示順';
...
add_filter('manage_course_posts_columns', 'fascina_add_course_columns');

// サブカテゴリー一覧のカラム内容を表示
function fascina_course_column_content($column_name, $post_id) {
...
add_action('manage_course_posts_custom_column', 'fascina_course_column_content', 10, 2);
...
// 工程表一覧のカラムをカスタマイズ
function fascina_add_process_chart_columns($columns) {
    $new_columns = array();
...
add_filter('manage_process_chart_posts_columns', 'fascina_add_process_chart_columns');

// 工程表一覧のカラム内容を表示
function fascina_process_chart_column_content($column_name, $post_id) {
...
add_action('manage_process_chart_posts_custom_column', 'fascina_process_chart_column_content', 10, 2);
...
function fascina_sortable_course_columns($columns) {
    $columns['menu_order'] = 'menu_order';
    return $columns;
}
add_filter('manage_edit-course_sortable_columns', 'fascina_sortable_course_columns');

// バナーの表示順列をソート可能にする
function fascina_sortable_banner_columns($columns) {
    $columns['menu_order'] = 'menu_order';
    return $columns;
}
add_filter('manage_edit-banner_sortable_columns', 'fascina_sortable_banner_columns');

// ネイリストの表示順列をソート可能にする
function fascina_sortable_nailist_columns($columns) {
    $columns['menu_order'] = 'menu_order';
    return $columns;
}
add_filter('manage_edit-nailist_sortable_columns', 'fascina_sortable_nailist_columns');
...
function fascina_add_gallery_filters() {
    ...
    if ($typenow === 'gallery') {
        ...
        if($current_main){
            $sub_categories = fascina_get_course_choices($current_main);
...
function fascina_add_course_filters() {
    global $typenow;
    if ($typenow === 'course') {
        // メインカテゴリーの選択肢
        $main_categories = array(
...
add_action('restrict_manage_posts', 'fascina_add_course_filters');
...
// サブカテゴリーのフィルター条件を適用
function fascina_apply_course_filters($query) {
    global $pagenow, $typenow;
...
add_action('pre_get_posts', 'fascina_apply_course_filters');
...
function fascina_get_gallery_page_posts($main_category = '', $sub_category = '', $posts_per_page = 20, $paged = 1) {
    ...
    if (!empty($sub_category) && $sub_category !== 'bridal') {
        ...
        if ($main_category === 'guest' && $sub_category === 'list') {
            ...
            $guest_sub_categories = fascina_get_course_choices('guest');
...
// トップページのクーポン表示制御
function fascina_get_top_coupon_posts($limit = 12) {
    $args = array(
        'post_type' => 'coupon',
        'posts_per_page' => $limit,
        'meta_query' => array(
...
function fascina_coupon_nailist_single_select_script() {
    ?>
...
add_action('admin_footer', 'fascina_coupon_nailist_single_select_script');

// ACFフィールドの選択肢を動的に更新
function fascina_update_coupon_nailist_choices($field) {
    if ($field['name'] === 'coupon_nailist') {
        $field['choices'] = fascina_get_nailist_choices_for_registration();
    }
    return $field;
}
add_filter('acf/load_field/name=coupon_nailist', 'fascina_update_coupon_nailist_choices');
add_filter('acf/load_field/key=field_coupon_nailist', 'fascina_update_coupon_nailist_choices');
...
function fascina_admin_filters_script() {
    global $typenow;
    if ($typenow === 'gallery') {
        $current_main = isset($_GET['main_category']) ? $_GET['main_category'] : '';
        $current_sub = isset($_GET['sub_category']) ? $_GET['sub_category'] : '';
...
add_action('admin_footer-edit.php', 'fascina_admin_filters_script');

// サブカテゴリーのスラッグ更新時にギャラリーも自動更新
function fascina_update_gallery_sub_categories_on_course_slug_change($post_id) {
    if (get_post_type($post_id) !== 'course') {
        return;
    }
    
    // 無限ループを防ぐ
    remove_action('save_post', 'fascina_update_gallery_sub_categories_on_course_slug_change');
    ...
    // アクションを再度追加
    add_action('save_post', 'fascina_update_gallery_sub_categories_on_course_slug_change');
}
add_action('save_post', 'fascina_update_gallery_sub_categories_on_course_slug_change');

// AJAXハンドラー
function fascina_ajax_get_course_choices() {
    check_ajax_referer('get_course_choices', 'nonce');
   
    $main_type = sanitize_text_field($_POST['main_type']);
    $sub_categories = fascina_get_course_choices($main_type);
   
    wp_send_json_success($sub_categories);
}
add_action('wp_ajax_get_course_choices', 'fascina_ajax_get_course_choices');