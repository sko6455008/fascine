...
function saloni_change_title_placeholder($title, $post) {
    if ($post->post_type === 'gallery') {
        ...
        $title = 'SEOに適した画像タイトル入力(例：パラジェル専門店 saloni)';
...
add_action('init', 'saloni_register_coupon_post_type');
        ...
        acf_add_local_field_group(array(
            ...
            'fields' => array(
                array(
                    ...
                    'type' => 'text',
                    'required' => 1,
                    'instructions' => '例: https://saloni.jp/coupon/tanaka/',
                ),
            ),
            'location' => array(
                array(
        ...
        acf_add_local_field_group(array(
            ...
            'fields' => array(
                array(
                    ...
                    'choices' => saloni_get_nailist_choices_for_registration(),
                    'return_format' => 'value',
                    'layout' => 'vertical'
                ),
                array(
...
function saloni_load_gallery_sub_category_choices($field) {
    if ($field['name'] === 'gallery_sub_category') {
        ...
        if ($main_category) {
            ...
            $field['choices'] = saloni_get_course_choices('hand');
        }
    }
    return $field;
}
...
add_action('before_delete_post', 'saloni_remove_nailist_from_coupons_on_delete');
...
add_action('before_delete_post', 'saloni_clear_galleries_on_subcategory_delete');
...
// ブログアーカイブの表示件数を変更
function saloni_blog_query_settings($query)
{
...
function saloni_gallery_column_content($column_name, $post_id) {
    if ($column_name === 'thumbnail') {
        ...
        if ($main_category && $sub_category !== null) {
            echo esc_html(saloni_get_sub_category_display_name_by_value($main_category, $sub_category));
        }
...
function saloni_coupon_column_content($column_name, $post_id) {
    if ($column_name === 'thumbnail') {
        ...
        echo esc_html(saloni_get_nailist_display_name_by_value($nailist_value));
    } elseif ($column_name === 'menu_order') {
        $post = get_post($post_id);
        echo $post->menu_order;
...
add_action('manage_nailist_posts_custom_column', 'saloni_nailist_column_content', 10, 2);
...
add_filter('acf/load_field/name=coupon_nailist', 'saloni_update_coupon_nailist_choices');
